...
ggplot(sunapee, aes(x=year, y=ice_off_doy)) +
  geom_point() +
  geom_line() +
  labs(title='Lake Sunapee Ice Off Day Over Time', x='Year', y='Day Of Year')
```
...
**Answer 2:**
Answer: I expect to see a negative relationship between Year and Ice Off Day because the average Ice Off Day of each year seems to decrease as the year increases. 
...
**Answer 3:**
We have a slope of -0.106 which is negative exactly like we expected. The slope is much more gradual than I expected. 
...
R2 is 0.205. Which means the year accounts for 20.5% of the variation in Ice Off Day. This means there is a relationship, but the relationship is not very strong. 
...
The residuals do not meet the assumptions of linear regression. They have a center around zero, but are skewed to the left, meaning the residuals do not follow a normal distribution. 
...
Our p-value is 2.5e-09 or 0.0000000025. 
...
The slope is -0.106 which means the ice off day of year decreases by about 0.106 days per year. Therefore, it would take approximately 67 years for the ice off day to have a one week change. This is meaningful because the average person is not concerned about a -0.106 day change for each year. People respond to and understand standard time periods, like a week, more than just percentages of a day. 
...
With alpha = 0.05, our p-value of 2.5e-9 is statistically significant. So we are certain this relationship does exist, however the R squared value of 0.205 means only 20.5% of the variation in the ice off day can be explained by the year. This means there are more unknown variables causing the other 79.5% of variation in ice off day. In ecology, a low R squared is always expected because the Earth is a very complex system that we can't fully model so I am comfortable using this model for additional analysis as long as the limitations are understood. 
...
```{r}
pred2040 <- fit$coefficients[1] + 2040 * fit$coefficients[2]

lower <- pred2040 + quantile(sunapee$resid, 0.05)
upper <- pred2040 + quantile(sunapee$resid, 0.95)
...
upper - lower
```
The predicted ice off day for 2040 is 102.6. We are 90% confident that the ice off day will be between days 83.2 and 116.8. 

The confidence interval is 33.6 days wide. 
...
sunapee <- sunapee |>
  mutate(era = ifelse(year >= 1970,"Later (1970 - Present)", "Early (1869-1969)"))

fit_early <- lm(ice_off_doy ~ year, data = sunapee |> filter(era == "Early (1869-1969)"))
fit_later <- lm(ice_off_doy ~ year, data = sunapee |> filter(era != "Early (1869-1969)"))
...
ggplot(sunapee, aes(x = year, y = ice_off_doy)) +
  ...
  geom_abline(intercept = fit_early$coefficients[1], slope = fit_early$coefficients[2], color = 'blue') +
  geom_abline(intercept = fit_later$coefficients[1], slope = fit_later$coefficients[2], color = 'red') +
  labs(x = "Year", y = "Ice Off Day", title='Ice Off Day By Year') 

fit_together <- lm(ice_off_doy ~ year + era + year:era, data = sunapee)
summary(fit_together)

fit_early$coefficients[2]
fit_later$coefficients[2]

```
I am confident that the ice off day is changing more quickly in recent years. When we plot the two time periods together we see that 1970-present has a much steeper negative slope than 1869-1969. The early period has a slope of -0.110 while the later period has a slope of -0.30. With a p-value of 0.023 the additional change in slope for the later period is statistically significant. 
